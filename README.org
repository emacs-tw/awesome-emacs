...
*** Massive Edit
    ...
    - [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Query-Replace.html][query-replace]] - =[built-in]= =query-replace= supports replacing text with regular text, while =query-replace-regexp= uses regular expressions. No visual feedback when replacing.
